 ...
 * @extends WC_Widget
 */
class Mixxcazt_Widget_Product_Brands extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__('A list or dropdown of product brands.', 'mixxcazt');
		$this->widget_id          = 'woocommerce_product_brands';
		$this->widget_name        = esc_html__('Product Brands', 'mixxcazt');
		$this->settings           = array(
			'title'              => array(
				'type'  => 'text',
				'std'   => esc_html__('Product Brands', 'mixxcazt'),
				'label' => esc_html__('Title', 'mixxcazt'),
			),
			'dropdown'           => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show as dropdown', 'mixxcazt'),
			),
			'show_logo'          => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show logo brand', 'mixxcazt'),
			),
			'count'              => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show product counts', 'mixxcazt'),
			),
			'hierarchical'       => array(
				'type'  => 'checkbox',
				'std'   => 1,
				'label' => esc_html__('Show hierarchy', 'mixxcazt'),
			),
			'show_children_only' => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Only show children of the current brand', 'mixxcazt'),
			),
			'hide_empty'         => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Hide empty brands', 'mixxcazt'),
			),
			'max_depth'          => array(
				'type'  => 'text',
				'std'   => '',
				'label' => esc_html__('Maximum depth', 'mixxcazt'),
			),
		);
	...
	public function widget($args, $instance) {
		...
		if ($dropdown) {
			...
			$list_args['walker']                  = new Mixxcazt_Product_Brand_List_Walker;
			$list_args['title_li']                = '';
			$list_args['pad_counts']              = 1;
			$list_args['show_option_none']        = esc_html__('No product brands exist.', 'mixxcazt');
			...
			echo '<ul class="product-brands" id="mixxcazt-brands-' . $id . '">';
...
add_action('widgets_init', function () {
	register_widget('Mixxcazt_Widget_Product_Brands');
});

add_action('init', function () {
	$labels = array(
		'name'                       => esc_html__('Brands', 'mixxcazt'),
		'singular_name'              => esc_html__('Brands', 'mixxcazt'),
		'menu_name'                  => esc_html__('Brands', 'mixxcazt'),
		'all_items'                  => esc_html__('All Brands', 'mixxcazt'),
		'parent_item'                => esc_html__('Parent Brand', 'mixxcazt'),
		'parent_item_colon'          => esc_html__('Parent Brand:', 'mixxcazt'),
		'new_item_name'              => esc_html__('New Brand Name', 'mixxcazt'),
		'add_new_item'               => esc_html__('Add New Brands', 'mixxcazt'),
		'edit_item'                  => esc_html__('Edit Brand', 'mixxcazt'),
		'update_item'                => esc_html__('Update Brand', 'mixxcazt'),
		'separate_items_with_commas' => esc_html__('Separate Brand with commas', 'mixxcazt'),
		'search_items'               => esc_html__('Search Brands', 'mixxcazt'),
		'add_or_remove_items'        => esc_html__('Add or remove Brands', 'mixxcazt'),
		'choose_from_most_used'      => esc_html__('Choose from the most used Brands', 'mixxcazt'),
	);
	$args   = array(
		'labels'            => $labels,
		'hierarchical'      => true,
