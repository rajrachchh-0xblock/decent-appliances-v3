 ...
 */
class Mixxcazt_Woocommerce_Extra {
    ...
    public function add_deal_fields() {
        ...
        <p class="form-field _deal_quantity_field">
            <label for="_sale_quantity"><?php esc_html_e('Sale quantity', 'mixxcazt') ?></label>
            <?php echo wc_help_tip(__('Set this quantity will make the product to be a deal. The sale will end when this quantity is sold out.', 'mixxcazt')); ?>
            ...
            <?php
            if ($quantity > 0) {
                echo '<span class="deal-sold-counts"><strong>' . sprintf(_n('%s product is sold', '%s products are sold', max(1, $sales_counts), 'mixxcazt'), $sales_counts) . '</strong></span>';
            }
            ?>
        </p>
    ...
    public function quantity_input_args($args, $product) {
        if (!mixxcazt_woocommerce_is_deal_product($product)) {
            return $args;
        }
...
new Mixxcazt_Woocommerce_Extra;
